...
```bash
gh release download v0.1.0 --repo kraftaa/whyslow --dir /tmp/whyslow-v0.1.0
cd /tmp/whyslow-v0.1.0
sha256sum --check SHA256SUMS
tar -xzf whyslow-0.1.0.tar.gz
```
...
sudo python3 -m venv /opt/whyslow/releases/0.1.0
sudo /opt/whyslow/releases/0.1.0/bin/python -m pip install --upgrade pip
sudo /opt/whyslow/releases/0.1.0/bin/python -m pip install \
  'whyslow[cloudwatch] @ file:///tmp/whyslow-v0.1.0/whyslow-0.1.0-py3-none-any.whl'
sudo ln -sfn /opt/whyslow/releases/0.1.0 /opt/whyslow/venv
/opt/whyslow/venv/bin/whyslow --version
```

Install the units from the extracted `whyslow-0.1.0/deploy/` directory into
...
push a matching tag such as `v0.1.0`. The release workflow rejects a tag that
does not exactly match the package version. It builds and checks the wheel and
source distribution, runs the isolated wheel smoke test, creates
`SHA256SUMS`, and attaches all three files to the GitHub release.
