...
```bash
gh release download v0.4.1 --repo kraftaa/whyslow --dir /tmp/whyslow-v0.4.1
cd /tmp/whyslow-v0.4.1
sha256sum --check SHA256SUMS
tar -xzf whyslow_db-0.4.1.tar.gz
```
...
sudo python3 -m venv /opt/whyslow/releases/0.4.1
sudo /opt/whyslow/releases/0.4.1/bin/python -m pip install --upgrade pip
sudo /opt/whyslow/releases/0.4.1/bin/python -m pip install \
  'whyslow-db[cloudwatch] @ file:///tmp/whyslow-v0.4.1/whyslow_db-0.4.1-py3-none-any.whl'
sudo ln -sfn /opt/whyslow/releases/0.4.1 /opt/whyslow/venv
/opt/whyslow/venv/bin/whyslow --version
```

Install the units from the extracted `whyslow_db-0.4.1/deploy/` directory into
...
push a matching tag such as `v0.4.1`. The release workflow rejects a tag that
