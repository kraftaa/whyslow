...
def build_timeline(store, start_ts, end_ts):
    ...
    for minute_bucket, category, n, first_ts, last_ts in store.sessions_aggregated_in(
        start_ts, end_ts
    ):
        timeline.append(
            (
    ...
    for row in store.blocking_edges_in(start_ts, end_ts):
        ...
        if ended_ts is None:
            coverage_gap_ts = store.collector_coverage_gap_after("postgres", ts, end_ts)
        ...
        active_at_window_end = effective_ended_ts is None or effective_ended_ts > end_ts
    ...
    for ts, host, backlog, pool_capacity, max_threads, running, rss_mb in store.puma_stats_in(
        start_ts, end_ts
    ):
    ...
    for ts, metric, value, source_instance in store.cloudwatch_metrics_in(start_ts, end_ts):
        ...
        timeline.append(
            (
                ts,
                f"CloudWatch {metric}: {value}{source}",
                {
                    "kind": "cloudwatch",
                    "metric": metric,
                    "value": value,
...
def check_blocking_signals(candidate_key, candidate_data, timeline):
    ...
    if any(app and app != "unknown host" for app in blocked_apps):
        signals.append(
            f"blocked sessions tagged to host(s): {', '.join(a for a in blocked_apps if a)}"
        )
...
def check_resource_signals(timeline, resource_buckets):
    ...
    for ts, _, meta in timeline:
        if meta.get("kind") == "cloudwatch" and meta.get("metric") == "CPUUtilization":
            if meta["value"] >= CPU_ALERT_THRESHOLD and any(
                _cloudwatch_overlaps_spike(ts, spike) for spike in resource_spikes
            ):
    ...
    if cw_alert:
        ...
        signals.append(
            f"CloudWatch CPUUtilization {cw_alert[1]:.0f}%{source} at t={cw_alert[0]:.0f}"
        )
    ...
    resource_reference_ts = [
        ts for spike in resource_spikes for ts in (spike["start_ts"], spike["end_ts"])
    ]
...
def explain(store, start_ts, end_ts):
    ...
    for (pid, usename), data in blocking_candidates.items():
        ...
        if signals:
            label = (
                label_query(data["edges"][0][1].get("blocking_query")) if data["edges"] else None
            )
            ...
            still_active = any(m.get("active_at_window_end") for _, m in data["edges"])
            currently_unresolved = any(m.get("currently_unresolved") for _, m in data["edges"])
            resolution_unknown = any(m.get("resolution_unknown") for _, m in data["edges"])
...
COLLECTOR_ROLES = [
    ("postgres", lambda n: n == "postgres", "blocking chains and database session pressure"),
    ("puma", lambda n: n.startswith("puma:"), "web app saturation (thread pool and backlog)"),
    ("cloudwatch", lambda n: n == "cloudwatch", "instance CPU and connection counts"),
]
...
def _assess_coverage(raw, db_instance_role=None):
    ...
    for role, matcher, describes in COLLECTOR_ROLES:
        ...
        matching_names.update(name for name in expected_collectors if matcher(name))
        matching = {
        ...
        fraction = min(fractions, default=0.0) if role == "puma" else max(fractions, default=0.0)
        missing_collectors = sorted(
            name for name, detail in matching.items() if detail["fraction"] < COVERAGE_OK_THRESHOLD
        )
...
def _significant_timeline(timeline):
    ...
    for ts, text, meta in timeline:
        ...
        if kind == "session":
            ...
            if (
                meta.get("metric") == "CPUUtilization"
                and meta.get("value", 0) >= CPU_ALERT_THRESHOLD
            ):
                significant.append((ts, text, meta))
        else:
...
def render(result, start_ts, end_ts):
    ...
    if gaps:
        ...
        for role, info in gaps:
            if info.get("note"):
                ...
                lines.append(
                    f"  {role:<11} missing {missing_pct:.0f}% of window "
                    f"-> cannot rule out: {info['describes']}"
                )
                if info.get("missing_collectors"):
                    lines.append(
                        f"  {'':<11} missing collectors: {', '.join(info['missing_collectors'])}"
                    )
    ...
    if len(display) > MAX_TIMELINE_LINES:
        ...
        lines.append(
            f"  ... {omitted} earlier timeline entries omitted "
    ...
    if counts:
        ...
        lines.append(
            "  (root-cause edges below identify the one true blocker, not every waiting session --"
        )
    ...
    for c in result["contributors"]:
        ...
        lines.append(
            f"- {c['name']} [{c['category']}] -- {c['confidence']} confidence "
    ...
    ongoing = [
        c
        for c in result["contributors"]
        if c["category"] == "blocking" and c.get("currently_unresolved") and c.get("pid")
    ]
    if ongoing:
        lines.append("")
