"""Reset the invalid-index scenario."""
...
def reset(ctx: common.Context) -> dict:
    ...
    if common.use_docker():
        ...
        try:
            conn = common.connect(ctx.config.admin_dsn("whyslow_invalid_index_reset"))
            try:
                with conn.cursor() as cur:
                    cur.execute("DROP TABLE IF EXISTS deployment_orders CASCADE")
            finally:
                conn.close()
