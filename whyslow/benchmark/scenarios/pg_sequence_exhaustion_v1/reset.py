"""Reset the exhausted-sequence scenario."""
...
def reset(ctx: common.Context) -> dict:
    ...
    if common.use_docker():
        ...
        try:
            conn = common.connect(ctx.config.admin_dsn("whyslow_sequence_reset"))
            try:
                with conn.cursor() as cur:
                    cur.execute("DROP TABLE IF EXISTS invoices CASCADE")
                    cur.execute("DROP SEQUENCE IF EXISTS invoice_id_seq")
            finally:
                conn.close()
